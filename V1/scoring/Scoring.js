export class Scoring {
  ...
  lastResult = null;
  constructor(options) {
    if (!options || typeof options !== 'object') throw new Error('Scoring: options object is required');
    const { url, apiKey, userId = null, itemIds = [], origin = 'sdk', log, show, mode, captures } = options;
    if (typeof url !== 'string' || !url.trim()) throw new Error('Scoring: options.url is required and must be a non-empty string');
    if (typeof apiKey !== 'string' || !apiKey.trim()) throw new Error('Scoring: options.apiKey is required and must be a non-empty string');
  ...
  getEndpoint() {
    if (!this._modelEndpoint || !this._modelEndpoint.trim()) throw new Error('Scoring.getEndpoint: model endpoint must be set (call model(endpoint) first)');
  ...
  async execute() {
    if (this._mode === 'match') {
      // Scoring is not part of a notification algo. In match mode, return an
      // identity score for every item so the algo's downstream stages run cleanly.
      return this._itemIds.map((id) => ({ id, score: 1 }));
    }
    if (!this._modelEndpoint || !this._modelEndpoint.trim()) throw new Error('Scoring.execute: model endpoint must be set (call model(endpoint) first)');
    if (!this._userId || typeof this._userId !== 'string' || !this._userId.trim()) throw new Error('Scoring.execute: user_id must be set (pass to constructor or call userId(id))');
    if (!Array.isArray(this._itemIds) || this._itemIds.length === 0) throw new Error('Scoring.execute: item_ids must be set and non-empty (pass to constructor or call itemIds([...]))');
